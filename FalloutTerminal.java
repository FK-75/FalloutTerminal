...
public class FalloutTerminal {
	...
	public String buildDisplayLine(int startAddress, float wordProbability) {
		...
		// Address 1
		String address = "0x";
		address = address + String.valueOf(Integer.toHexString(startAddress)).toUpperCase();
		///////////////////////////////////////////////////////////////////////////////
	
	    String nonalphanumeric = "!@#&()�[{}]:;',?/*`~$^+=<>\"";
	    String line = "";
	    String line2 = "";
	    // If word isn't needed
	    if(wordProbability>0.6) {
		    for (int i = 0; i < 30; i++) {
		        line = line + nonalphanumeric.charAt(rand.nextInt(nonalphanumeric.length())); 
		        }
		    
		    for (int i = 0; i < 30 ;i++) {
		        line2 = line2 + nonalphanumeric.charAt(rand.nextInt(nonalphanumeric.length())); 
		        }
	    }
	    
	    // If word is needed
	    else {
	    	String wordToAdd=words[rand.nextInt(words.length)];
		    ...
		    // Add word to line 1
		    int startIndex=rand.nextInt(30-wordToAdd.length());
		    line=line.substring(0,startIndex)+wordToAdd+line.substring(startIndex+8);
		    addSelectedWord(wordToAdd);
		    
		    // Add word to line 2
		    startIndex=rand.nextInt(30-wordToAdd.length());
		    wordToAdd=words[rand.nextInt(words.length)];
		    addSelectedWord(wordToAdd);
		    line2=line2.substring(0,startIndex)+wordToAdd+line2.substring(startIndex+8);
		    }
	    /////////////////////////////////////////////////////////////////////////////////
	    /// Address 2
	    String address_2 = "0x";
		address_2 = address_2 + String.valueOf(Integer.toHexString(startAddress+240)).toUpperCase();
		////////////////////////////////////////////////////////////////////////////////
	    /// Final Output
		String finalOutput = address + " " + line + " " + address_2 + " " + line2;
